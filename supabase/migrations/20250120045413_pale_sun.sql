/*
  ...
  1. Tables
    - api_keys: Store user API keys
    - discussions: Store discussion data
    - messages: Store discussion messages
    - admin_users: Store admin user data
  ...
  3. Indexes
    ...
    - Add GIN indexes for JSON/array columns
*/
...
-- Add expert_ids array column separately to avoid constraint issues
ALTER TABLE discussions 
ADD COLUMN expert_ids text[] NOT NULL DEFAULT '{}';

-- Add expert constraints after column exists
ALTER TABLE discussions
ADD CONSTRAINT min_experts CHECK (expert_ids IS NULL OR array_length(expert_ids, 1) IS NULL OR array_length(expert_ids, 1) >= 1),
ADD CONSTRAINT max_experts CHECK (expert_ids IS NULL OR array_length(expert_ids, 1) IS NULL OR array_length(expert_ids, 1) <= 3);
